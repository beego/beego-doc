...
如果不确定究竟注册了什么路由，可以参考[如何查看我注册的路由信息](router_tree.md)

更多路由规则和路径匹配的问题，可以参考[路由规则与路径匹配](router_rule.md)
...
```
需要注意的是，我们新的注册方法，要求我们传入方法的时候，传入的一定是`(*YourController).MethodName`。之前我们提过，处理`http`请求的方法，其接收器应该是一个指针，因此这里我们要用指针的写法。

要额外引起注意的是`RouterAny`方法，这意味着，任意的`http`方法都可以被处理。
